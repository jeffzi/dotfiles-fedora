"""https://github.com/qtile/qtile/pull/2606"""
...
class MonadThreeCol(MonadTall):
    ...
    def cmd_normalize(self, redraw=True):
        ...
        if self.screen_rect is not None:
            ...
            if left_count > 0:
                self.relative_sizes += self._split_integer(height, left_count)
            if right_count > 0:
                self.relative_sizes += self._split_integer(height, right_count)
    ...
    def _maximize_secondary(self):
        ...
        max_height = self.screen_rect.height - (
            (column.count - 1) * self.min_secondary_size
        )
        for i in range(column.start, column.end):
            self.relative_sizes[i] = (
                max_height if i == focused else self.min_secondary_size
            )
